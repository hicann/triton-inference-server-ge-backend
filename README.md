...
## 新版本特性
1. 支持从onnx文件读取模型输入输出信息；   
2. 调整动态图在多实例下使用多Session方式，提高并发吞吐(显存占用会增高)
3. 补充调优方法论以及cnclip模型的最佳实践
...
* 非0轴动态：若output中包含动态轴，在导出onnx时需指定其与input中轴的关系，详情请查看[模型转换为onnx](#执行推理)
...
input、output 若不填写，则程序会尝试从onnx文件中读取，支持动态轴方式，但要满足限制条件，详情请参考 [模型转换为onnx](#模型转换为onnx)   
