...
* 根据规范，若config中配置了max_batch_size>0 则默认0轴为BS轴，在配置中无需配置BS轴，比如input0的shape[-1, 3, 224, 224]，若配置了max_batch_size: 8,则input0 的shape配置为[3, 224, 224]; 若max_batch_size=0 或未配置，则用户需要明确bs轴为固定值或 -1。
...
input、output 若不填写，则程序会尝试从onnx文件中读取，支持动态轴方式，但要满足限制条件，详情请参考 [Torch模型转换为onnx](#Torch模型转换为onnx)。 <font color="#dd0000">Tensorflow 目前不支持自动获取。</font>   
